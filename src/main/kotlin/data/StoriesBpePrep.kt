...
object StoriesBPEPrep {
    ...
    fun writeData(data: List<Int>, file: File) {
        println("data size: ${String.format("%,d", data.size)}")
        val buffer = ByteBuffer.allocate(data.size * 4)
        for (d in data) {
            buffer.putInt(d)
        }
        buffer.flip()
        FileChannel.open(file.toPath(), StandardOpenOption.CREATE, StandardOpenOption.WRITE, StandardOpenOption.TRUNCATE_EXISTING)
            .use { channel ->
