 ...
 */
class ScalarFeedForward(modelConfig: GPTConfig) {
    /** 확장 레이어: 임베딩 차원을 4배로 확장 */
    private val expansionLayer = ScalarLinear(modelConfig.embeddingDimension, 4 * modelConfig.embeddingDimension, modelConfig.useBias)

    /** 수축 레이어: 4배 확장된 차원을 원래 차원으로 복원 */
    private val contractionLayer = ScalarLinear(4 * modelConfig.embeddingDimension, modelConfig.embeddingDimension, modelConfig.useBias)

    /** 정규화를 위한 Dropout 레이어 */
    private val mlpDropout = ScalarDropout(modelConfig.dropoutProbability)
