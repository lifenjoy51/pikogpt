...
class Value(
    ...
    operator fun times(other: Number): Value = this * Value(other.toFloat())

    operator fun unaryMinus(): Value = this * -1.0f

    operator fun minus(other: Value): Value = this + -other

    operator fun minus(other: Number): Value = this - Value(other.toFloat())
    ...
    fun gelu(): Value {
        ...
        val x = this
        val c1 = Value(sqrt(2.0 / PI).toFloat())
        val c2 = Value(0.044715f)

        val inner = c1 * (x + c2 * x.pow(3.0f))
        // Kotlin의 tanh는 Value 객체에 대해 정의되어 있지 않으므로, tanh를 직접 구현하거나 근사해야 합니다.
        // 여기서는 tanh(y) = (exp(2y) - 1) / (exp(2y) + 1) 공식을 사용합니다.
        val exp2y = (inner * 2.0f).exp()
        val tanhInner = (exp2y - 1.0f) / (exp2y + 1.0f)

        return x * 0.5f * (1.0f + tanhInner)
    }
