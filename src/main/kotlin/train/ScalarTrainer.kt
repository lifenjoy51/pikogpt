 ...
 */
class ScalarTrainer(private val config: TrainConfig) {
    /** GPT 모델 인스턴스 */
    private lateinit var model: ScalarPikoGPT

    /** AdamW 옵티마이저 인스턴스 */
    private lateinit var optimizer: ScalarAdamW
    ...
    private fun initModel() {
        ...
        when (config.initFrom) {
            "scratch" -> {
                println("새 모델 초기화")
                model = ScalarPikoGPT(modelConfig)
            }
    ...
    private fun getVocabularySize(): Int {
        ...
        return metadata.vocabularySize
    }
    ...
    private fun evaluateBatch(inputSequences: Array<IntArray>, targetSequences: Array<IntArray>): Double {
        ...
        gpt.ScalarDropout.training = false
    ...
    private fun saveCheckpoint(loss: Double, isBest: Boolean) {
        ...
        // 체크포인트 생성
        val checkpoint = ScalarCheckpoint(
    ...
    private fun extractModelState(): ModelState {
        ...
        return ModelState(
            tokenEmbedding = List(model.config.vocabularySize) {
                List(model.config.embeddingDimension) { Random.nextDouble(-0.1, 0.1) }
            },
            positionEmbedding = List(model.config.maxSequenceLength) {
                List(model.config.embeddingDimension) { Random.nextDouble(-0.1, 0.1) }
            },
            blocks = List(model.config.numberOfLayers) {
                BlockState(
                    firstLayerNorm = LayerNormState(
                        weight = List(model.config.embeddingDimension) { 1.0 },
                        bias = if (model.config.useBias) List(model.config.embeddingDimension) { 0.0 } else null
                    ),
                    attention = AttentionState(
                        queryProjection = createLinearState(model.config.embeddingDimension, model.config.embeddingDimension),
                        keyProjection = createLinearState(model.config.embeddingDimension, model.config.embeddingDimension),
                        valueProjection = createLinearState(model.config.embeddingDimension, model.config.embeddingDimension),
                        outputProjection = createLinearState(model.config.embeddingDimension, model.config.embeddingDimension)
                    ),
                    ...
                    feedForward = FeedForwardState(
                        fullyConnected = createLinearState(model.config.embeddingDimension, 4 * model.config.embeddingDimension),
                        projection = createLinearState(4 * model.config.embeddingDimension, model.config.embeddingDimension)
                    )
                )
            },
            lmHead = createLinearState(model.config.embeddingDimension, model.config.vocabularySize)
        )
    }
    ...
    private fun loadCheckpoint() {
        ...
        val jsonParser = Json { ignoreUnknownKeys = true }
        val checkpoint = jsonParser.decodeFromString<ScalarCheckpoint>(checkpointFile.readText())

        // 모델 설정 확인
        val modelConfiguration = checkpoint.modelArgs
        model = ScalarPikoGPT(modelConfiguration)
