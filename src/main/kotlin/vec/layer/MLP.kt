...
/**
 * Transformer FFN 블록: expand → GELU → contract.
 *
 *   x [N, C] --fc--> h [N, 4C] --gelu--> a [N, 4C] --proj--> y [N, C]
 *
 *   Forward:  h = fc(x);  a = gelu(h);  y = proj(a)
 *   Backward: 체인 룰을 그대로
 *     da = proj.backward(gy)              → ∂L/∂a
 *     dh = geluBackward(cachedH, da)      → ∂L/∂h
 *     dx = fc.backward(Tensor(...,dh))    → ∂L/∂x
 *
 * 각 하위 레이어가 자체 param grad를 누적하므로 여기선 따로 관여할 일 없음.
 */
class MLP(val embedDim: Int, useBias: Boolean = true) {
