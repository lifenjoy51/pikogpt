 ...
 *
 *   Forward 개요:
 *     1) 입력 x[T, C]로부터 Q, K, V 각각 [T, C] 생성 (세 개의 VecLinear 투영)
...
class VecSelfAttention(
    ...
    val qProjection: VecLinear = VecLinear(embedDim, embedDim, useBias)
    val kProjection: VecLinear = VecLinear(embedDim, embedDim, useBias)
    val vProjection: VecLinear = VecLinear(embedDim, embedDim, useBias)
    val outputProjection: VecLinear = VecLinear(embedDim, embedDim, useBias)
     ...
     * 표준 GPT-2 convention.
     */
    val attnDropout: VecDropout = VecDropout(dropoutProbability)
    val residDropout: VecDropout = VecDropout(dropoutProbability)
    ...
    fun forward(x: Tensor): Tensor {
        ...
        // 2) Attention dropout — attn_probs 전체를 [H*T*T] 평탄 Tensor로 감싸 VecDropout에 위임.
