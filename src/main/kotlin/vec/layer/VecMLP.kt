 ...
 *   - 3개 VecLinear 사용 (gate, up, down) — params는 GELU와 거의 같음
...
class VecMLP(
    ...
    val fullyConnected: VecLinear? =
        if (isSwiGLU) null else VecLinear(embedDim, hiddenDim, useBias)
    val projection: VecLinear? =
        if (isSwiGLU) null else VecLinear(hiddenDim, embedDim, useBias)

    // SwiGLU 경로 레이어
    val gateProjection: VecLinear? =
        if (isSwiGLU) VecLinear(embedDim, hiddenDim, useBias) else null
    val upProjection: VecLinear? =
        if (isSwiGLU) VecLinear(embedDim, hiddenDim, useBias) else null
    val downProjection: VecLinear? =
        if (isSwiGLU) VecLinear(hiddenDim, embedDim, useBias) else null

    val dropout: VecDropout = VecDropout(dropoutProbability)
